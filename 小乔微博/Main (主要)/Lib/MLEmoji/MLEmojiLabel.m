    ...
    if (result.resultType == NSTextCheckingTypeCorrection) {
        ...
        for (NSUInteger i=0; i<kURLActionCount; i++) {
            if ([result.replacementString hasPrefix:kURLActions[i]]) {
                ...
                if(self.emojiLabeldelegate&&[self.emojiLabeldelegate respondsToSelector:@selector(mlEmojiLabel:didSelectLink:withType:)]){
                    //type的数组和i刚好对应
                    [self.emojiLabeldelegate mlEmojiLabel:self didSelectLink:content withType:i];
                    return YES;
                }
                return NO;
            }
